...
THREE.Ray.prototype = {
	...
	intersectObject: function ( object ) {
		...
		if ( object instanceof THREE.Particle ) {
			...
			var sumScale = Math.max( object.scale.x, Math.max( object.scale.y, object.scale.z ));
			var checkObject = object;
			while( checkObject.parent && checkObject.parent instanceof THREE.Object3D ) {
				checkObject = checkObject.parent;
				sumScale *=  Math.max( checkObject.scale.x, Math.max( checkObject.scale.y, checkObject.scale.z ));
			}

			if ( ! distance || distance > object.geometry.boundingSphere.radius * sumScale ) {
				return [];
			}
